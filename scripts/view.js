...
function setupWindowScroll() {
    $(window).resize(function () {
        ...
        tranHeight = $(window).height() - tranOffset - 175;
		$("#tabs-b").height(tranHeight) 
		$("#tabs-transcript").height( $("#tabs-b").height() - 55);
		$("#transcript-content").height($("#tabs-b").height() - 55);
...
function renderCaptions(utterances) {
    ...
    $.each(utterances, function (uttIndex, utterance) {
        var out = '<tr><td class="pTime" sTime="' + utterance.startTime + '">' + formatTimeForDisplay(utterance.startTime) + ':</td><td>';
	...
	$(".pTime").dblclick(function(){
		playAudio( $(this).attr('sTime'));
	});
...
function playAudio(time) {
    ...
    player.jPlayer('play', parseInt(time));
}
